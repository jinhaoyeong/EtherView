...
export default function Transactions() {
  ...
  }, [walletAddress, settings?.display.itemsPerPage, settings?.display.isCustomItemsPerPage]);
  ...
  const loadTransactions = async (address: string, page: number = 1, append: boolean = false) => {
    try {
      ...
      // Use configured items per page or default to 15
      const itemsPerPage = settings?.display.isCustomItemsPerPage ? settings.display.itemsPerPage : 15;

      // ⚡ OPTIMIZED: Fetch paginated transaction data from the API
      const realTransactions = await WalletAPI.fetchTransactions(address, page, itemsPerPage);
      ...
      // If we got exactly the requested number of transactions, there might be more available
      setHasMore(realTransactions.length === itemsPerPage);
  ...
  const handleRefresh = async () => {
    if (walletAddress) {
      ...
      // 🗑️ Clear transaction cache to force fresh fetch
      WalletAPI.clearTransactionCache(walletAddress);
  ...
  useAutoRefresh(handleRefresh, !!walletAddress, 300); // 5 minutes default for transactions
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Tooltip>
            ...
            <TooltipContent>
              <p>{t('transactions.refresh_tooltip')}</p>
            </TooltipContent>
          </Tooltip>
        </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={filter} onValueChange={setFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="token_received">{t('transactions.token_received')}</SelectItem>
                  <SelectItem value="token_sent">{t('transactions.token_sent')}</SelectItem>
                  <SelectItem value="trade">{t('transactions.trade')}</SelectItem>
                </SelectContent>
              </Select>
            </div>
        ...
        {error ? (
          ...
          <Card className="bg-card border-border p-12 shadow-lg text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground mb-2">
              {searchQuery || filter !== 'all' ? t('transactions.no_transactions') : 'No transactions yet'}
            </h3>
          ...
          <div className="space-y-4">
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {settings?.display?.showTransactionFees && (
                      <div className="text-xs text-muted-foreground">
                        <div className="font-medium">{t('transactions.fee')}:</div>
                        ...
                        <div className="mt-2 font-medium">{t('transactions.gas_price')}:</div>
            ...
            {!loading && transactions.length > 0 && (
              <div className="text-center mt-4 text-sm text-muted-foreground">
                {t('transactions.showing').replace('{count}', filteredTransactions.length.toString()).replace('{total}', transactions.length.toString())}
